...
module extrusion(sidelen = 20, height = 100, notch= 6.2, border_thickness = 1.8, arm_thickness = 1.5,
                 center_diameter = 4.2, center_sidelen = 6.1, corner_inner = 0,
                 corner_outer = 4.5, center = false) {
...
module zext(height) {
    color("#3F3F3F") extrusion(height = height);
}
module yext(height) {
    translate([0, 0, ext_sidelen]) {
        rotate([-90, 0, 0]) {
            zext(height = height);
        }
    }
